package net.tiphainelaurent.buildersblocks.api;
...
public abstract class FamilyRegistry
{
    ...
    public void registerAll(final ItemGroup group)
    {
        ...
        ArtificeResourcePack resourcePack = Artifice
            .registerAssets(new Identifier(BuildersBlocks.MOD_ID, textureDirectory), pack -> {
                ...
                blockIds.forEach((blockId) -> {
                    ...
                    Block block = net.tiphainelaurent.buildersblocks.api.helpers.Block.builder(getAncestor())
                        .mineable()
                        .asItem(group)
                        .build(namespace, blockName);

                    final Identifier blockModelId = new Identifier(BuildersBlocks.MOD_ID, "block/" + blockName);
                    pack.addBlockState(blockId, state -> state
                        .variant("", variant -> variant
                            .model(blockModelId)));
                    pack.addBlockModel(blockId, model -> model
                        .parent(new Identifier("block/cube_all"))
                        .texture("all", new Identifier(BuildersBlocks.MOD_ID, String.format("block/%s/%s",
